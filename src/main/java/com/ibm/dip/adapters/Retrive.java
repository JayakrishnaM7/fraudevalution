...
@Component
public class Retrive implements JavaDelegate{

    @Autowired
    private Utility utility;

    @Autowired
    private RetrieveRestProcessor restProcessor;

    private final static Logger LOGGER = Logger.getLogger(Retrive.class.getName());
    ...
    public void execute(DelegateExecution execution) throws Exception {
         ...
         if(response instanceof ApiError ||  response instanceof GenericError){
            //Failure Path
            throw new RuntimeException("Error occurred in the adapter:"+Retrive.class.getName());
         }else{
