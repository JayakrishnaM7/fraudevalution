...
@Service
public class RetrieveRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(RetrieveRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.retrieve.hosturl}")
	private String url;
	
	@Value("${spring.api.retrieve.context-path}")
	private String contextPath;

	@Value("${spring.api.retrieve.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  RetrieveRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ RetrieveRestProcessor.class.getName());
