...
@Service
public class EvaluateRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(EvaluateRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.evaluate.hosturl}")
	private String url;
	
	@Value("${spring.api.evaluate.context-path}")
	private String contextPath;

	@Value("${spring.api.evaluate.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  EvaluateRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ EvaluateRestProcessor.class.getName());
